...
class QuotaService:
    ...
    # PERUBAHAN 1: Menambahkan 'ssh' ke protokol yang didukung
    SUPPORTED_PROTOCOLS = ["vmess", "vless", "trojan", "ssh"]
    DB_PATH = "/etc/vpn/database.db"
    
    def __init__(self):
        # Tidak lagi membuat koneksi di __init__ untuk menghindari masalah thread
        logger.info("QuotaService diinisialisasi. Koneksi ke database akan dibuat per permintaan.")
        
    def _convert_timestamp_to_datetime(self, timestamp):
        """
        Helper untuk mengkonversi Unix Epoch ke format YYYY-MM-DD HH:MM:SS.
        PERUBAHAN 2: Format tanggal sekarang lebih lengkap (termasuk jam).
        """
        if timestamp:
            try:
    ...
    def get_user_quota(self, protocol, username):
        ...
        Koneksi database dibuat secara lokal untuk memastikan thread-safety.
        """
        try:
            ...
            with sqlite3.connect(self.DB_PATH) as conn:
                ...
                # 1. Konversi Kuota dari Bytes ke GB (bulat)
                # PERUBAHAN 3: Kuota dibulatkan menjadi integer tanpa desimal.
                quota_limit_bytes = row['quota']
                quota_used_bytes = row['quota_usage']
                
                is_unlimited = quota_limit_bytes <= 0
                
                quota_limit_gb = "Unlimited" if is_unlimited else int(quota_limit_bytes // self.BYTES_TO_GB)
                quota_used_gb = int(quota_used_bytes // self.BYTES_TO_GB)
                
                if is_unlimited:
                    quota_remaining_gb = "Unlimited"
                else:
                    quota_remaining_bytes = quota_limit_bytes - quota_used_bytes
                    quota_remaining_gb = int(max(0, quota_remaining_bytes) // self.BYTES_TO_GB)

                # 2. Konversi Timestamp ke format datetime
                expiry_date = self._convert_timestamp_to_datetime(row['expired_at'])
                created_at_date = self._convert_timestamp_to_datetime(row['created_at'])
                ...
                # PERUBAHAN 4: Untuk SSH dan Trojan, gunakan field "password"
                if protocol in ["trojan", "ssh"]:
        ...
        try:
            ...
            with sqlite3.connect(self.DB_PATH) as conn:
                ...
                for user_row in users_data:
                    ...
                    # 2. Konversi Timestamp ke format datetime
                    expiry_date = self._convert_timestamp_to_datetime(user_row['expired_at'])
                    created_at_date = self._convert_timestamp_to_datetime(user_row['created_at'])
