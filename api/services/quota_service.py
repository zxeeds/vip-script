...
class QuotaService:
    ...
    SUPPORTED_PROTOCOLS = ["vmess", "vless", "trojan"] # Tambahkan "ssh" jika diperlukan
    DB_PATH = "/etc/vpn/database.db"
    
    def __init__(self):
        # Koneksi database dipindahkan ke setiap fungsi (per request)
        # untuk menghindari error thread-safety pada SQLite.
        logger.info("QuotaService siap digunakan. Koneksi database dibuka per request.")
    ...
    def get_user_quota(self, protocol, username):
        ...
        try:
            ...
            # --- PERUBAHAN: Koneksi Database Lokal (Thread-Safe) ---
            with sqlite3.connect(self.DB_PATH) as conn:
                ...
                if not row:
                    logger.warning(f"User {username} dengan protokol {protocol} tidak ditemukan atau tidak aktif.")
